    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <form class="mt-8 space-y-6" @submit.prevent="handleLogin">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="text-sm">
            <a href="#" class="font-medium text-accent-600 hover:text-accent-500 transition-colors">
              忘记密码？
            </a>
          </div>
        </div>
        ...
        <div class="relative">
          ...
          <div class="relative flex justify-center text-sm">
            <span class="px-2 bg-primary-50 text-primary-500">其他登录方式</span>
          </div>
        </div>
...
const handleLogin = async () => {
  ...
  if (success) {
    uiStore.addNotification('success', '登录成功')
    router.push('/chat')
  }
}

const handleSocialLogin = (provider: string) => {
  uiStore.addNotification('info', `${provider}登录功能即将推出`)
}
